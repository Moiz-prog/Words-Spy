...
public class WordChecker : MonoBehaviour
{
    ...
    private Ray _rayDiagonalLeftUp, _rayDiagnonalLeftDown;
    private Ray _rayDiagonalRightUp, _rayDiagnonalRightDown;
    ...
    private void Update()
    {
        if (_assignedPoints > 0 && Application.isEditor)
        {
            ...
            Debug.DrawRay(_rayDiagnonalLeftDown.origin, _rayDiagnonalLeftDown.direction * 4);
            Debug.DrawRay(_rayDiagonalRightUp.origin, _rayDiagonalRightUp.direction * 4);
            Debug.DrawRay(_rayDiagnonalRightDown.origin, _rayDiagnonalRightDown.direction * 4);
    ...
    private void SquareSelected(string letter, Vector3 squarePosition,  int squareIndex)
    {
        if (_assignedPoints == 0)
        {
            ...
            _rayDiagnonalLeftDown = new Ray(new Vector2(squarePosition.x, squarePosition.y), new Vector2(-1, -1));
            _rayDiagonalRightUp = new Ray(new Vector2(squarePosition.x, squarePosition.y), new Vector2(1, 1));
            _rayDiagnonalRightDown = new Ray(new Vector2(squarePosition.x, squarePosition.y), new Vector2(1, -1));
        }
