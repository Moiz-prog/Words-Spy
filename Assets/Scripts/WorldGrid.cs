...
public class WorldGrid : MonoBehaviour
{
    ...
    private void SetSquaresPosition()
    {
        ...
        {
            Debug.LogError("Square list is empty. Ensure SpawnGridSquare() is creating and adding squares correctly.");
            return;
        }
        ...
        if (currentGameData != null)
        {
            ...
            foreach (var squares in currentGameData.selectedBoardData.Board)
            {
                foreach (var squareLetter in squares.Row)
                {
                    ...
                    {
                        Debug.LogError("All fields in your array should have some letter. Press Fill up with random buttom in your board data to add random letter. Letter: " + squareLetter);
    ...
    private void DebugAlphabetData()
{
    foreach (var letterData in alphabetData.AlphabetNormal)
    {
        if (string.IsNullOrEmpty(letterData.letter) || letterData.image == null)
        {
            Debug.LogError($"AlphabetNormal contains an invalid entry: Letter='{letterData.letter}', Sprite={(letterData.image == null ? "null" : "assigned")}");
        }
    }

    foreach (var letterData in alphabetData.AlphabetHighlighted)
    {
        if (string.IsNullOrEmpty(letterData.letter) || letterData.image == null)
        {
            Debug.LogError($"AlphabetHighlighted contains an invalid entry: Letter='{letterData.letter}', Sprite={(letterData.image == null ? "null" : "assigned")}");
        }
    }

    foreach (var letterData in alphabetData.AlphabetWrong)
    {
        if (string.IsNullOrEmpty(letterData.letter) || letterData.image == null)
        {
            Debug.LogError($"AlphabetWrong contains an invalid entry: Letter='{letterData.letter}', Sprite={(letterData.image == null ? "null" : "assigned")}");
        }
    }
}
